...
describe("tests for divide operation", () => {
	const calculateDivision = (...values) => getOperationResult("divide", ...values);
	...
	it("should handle decimal numbers", () => {
		expect(calculateDivision(0.12313132, 0.3737373737)).toBeCloseTo(0.12313132 / 0.3737373737, ROUNDING_PRECISION);
		expect(calculateDivision(0.21312322, 0.2342342424)).toBeCloseTo(0.21312322 / 0.2342342424, ROUNDING_PRECISION);
		expect(calculateDivision(0.1, 0.2)).toBeCloseTo(0.1 / 0.2, ROUNDING_PRECISION);
	});

	it("should return correct divided value", () => {
		expect(calculateDivision(10, 20)).toBe(0.5);
		expect(calculateDivision("10", "20")).toBe(0.5);
		expect(calculateDivision(-2, -3)).toBeCloseTo(0.66666666666, ROUNDING_PRECISION);
		expect(calculateDivision(2, -3)).toBeCloseTo(-0.66666666666, ROUNDING_PRECISION);
	...
	it("should handle 1/x", () => {
		...
		expect(calculateDivision(1, -120)).toBeCloseTo(-0.00833333333, ROUNDING_PRECISION);
