...
describe('tests for root operation', () => {
	...
	test('should return correct square root for positive numbers', () => {
		expect(calculateRoot(4, 2)).toBeCloseTo(
			Math.sqrt(4),
			ROUNDING_PRECISION,
		);
		expect(calculateRoot('9', 2)).toBeCloseTo(
			Math.sqrt(9),
			ROUNDING_PRECISION,
		);
		expect(calculateRoot(16, '2')).toBeCloseTo(
			Math.sqrt(16),
			ROUNDING_PRECISION,
		);
		expect(calculateRoot('25', '2')).toBeCloseTo(
			Math.sqrt(25),
			ROUNDING_PRECISION,
		);
	});
	...
	test('should return correct cube root', () => {
		expect(calculateRoot(8, 3)).toBeCloseTo(
			Math.cbrt(8),
			ROUNDING_PRECISION,
		);
		expect(calculateRoot(27, 3)).toBeCloseTo(
			Math.cbrt(27),
			ROUNDING_PRECISION,
		);
		expect(calculateRoot(64, 3)).toBeCloseTo(
			Math.cbrt(64),
			ROUNDING_PRECISION,
		);
	});
	...
	test('should return correct x^y root', () => {
		expect(calculateRoot(125, 5)).toBeCloseTo(
			2.6265278044,
			ROUNDING_PRECISION,
		);
		expect(calculateRoot(8, -6)).toBeCloseTo(
			0.7071067811865475,
			ROUNDING_PRECISION,
		);
	});
	...
	test('should return approximate roots for non-perfect squares', () => {
		expect(calculateRoot(2, 2)).toBeCloseTo(
			Math.sqrt(2),
			ROUNDING_PRECISION,
		);
		expect(calculateRoot(10, 2)).toBeCloseTo(
			Math.sqrt(10),
			ROUNDING_PRECISION,
		);
		expect(calculateRoot(50, 2)).toBeCloseTo(
			Math.sqrt(50),
			ROUNDING_PRECISION,
		);
	});
